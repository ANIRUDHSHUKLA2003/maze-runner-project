...
if __name__=='__main__':
    myMaze=maze(50,50)
    myMaze.CreateMaze(25,13,loopPercent=100)
    # myMaze.CreateMaze(loadMaze='dijkMaze.csv')

    h1=agent(myMaze,14,14,filled=True,color=COLOR.red)
    h2=agent(myMaze,24,16,filled=True,color=COLOR.red)
    h3=agent(myMaze,44,19,filled=True,color=COLOR.red)
    h4=agent(myMaze,34,12,filled=True,color=COLOR.red)
    h5=agent(myMaze,42,43,filled=True,color=COLOR.red)
    ...
    a=agent(myMaze,6,1,shape='square',color=COLOR.blue,filled=True,footprints=True)
